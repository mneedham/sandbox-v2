    ...
    def do_GET(self):
        ...
        if 'sandbox' in queryParsed.keys():
            if 'SANDBOX_HASHKEY' in os.environ.keys() and queryParsed['sandbox'][0] == os.environ['SANDBOX_HASHKEY']:
                ...
                response = response + subprocess.check_output(["/var/lib/neo4j/bin/neo4j-backup", "-host", "localhost", "-to", "/db-backup/"])
                response = response + subprocess.check_output(["zip", "-r", "/db-backup.zip", "/db-backup"])
